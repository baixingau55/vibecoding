...
export function MessageCenter({
  ...
  const [appliedFilter, setAppliedFilter] = useState<FilterState>(DEFAULT_FILTER_STATE);
  const [draftFilter, setDraftFilter] = useState<FilterState>(DEFAULT_FILTER_STATE);
  ...
  const filteredMessages = useMemo(() => {
    return messages.filter((item) => {
      ...
      if (appliedFilter.status === "unread" && item.read) return false;
      if (appliedFilter.status === "read" && !item.read) return false;
      if (appliedFilter.day && item.createdAt.slice(0, 10) !== appliedFilter.day) return false;
      if (appliedFilter.hour && item.createdAt.slice(11, 13) !== appliedFilter.hour.padStart(2, "0")) return false;
      return true;
    });
  ...
  useEffect(() => {
    setDraftFilter((current) => ({ ...current, type: activeType }));
    setAppliedFilter((current) => ({ ...current, type: activeType }));
  }, [activeType]);
  ...
  function openFilterDrawer() {
    setDraftFilter({ ...appliedFilter, type: activeType });
    setFilterOpen(true);
  }

  function closeFilterDrawer(resetToDefault = false) {
    if (resetToDefault) {
      setDraftFilter(DEFAULT_FILTER_STATE);
      setAppliedFilter(DEFAULT_FILTER_STATE);
      setActiveType("all");
    } else {
      setDraftFilter(DEFAULT_FILTER_STATE);
    }
  ...
  function applyFilterDrawer() {
    ...
    setActiveType(draftFilter.type);
    setFilterOpen(false);
  }

  const unreadCountByType = (type: "all" | MessageItem["type"]) =>
    messages.filter((item) => !item.read && (type === "all" ? true : item.type === type)).length;
  ...
  return (
    <div className="ai-page ai-message-page">
      <section className="ai-panel ai-message-shell">
        <div className="ai-message-tabs">
          <button type="button" className={activeType === "all" ? "ai-message-tab ai-message-tab-active" : "ai-message-tab"} onClick={() => setActiveType("all")}>
            ...
            <span className="ai-message-tab-badge">{unreadCountByType("all")}</span>
          </button>
          {messageTypes.map((type) => (
            ...
            >
              ...
              <span className="ai-message-tab-badge">{unreadCountByType(type)}</span>
            </button>
          ))}
        </div>
        ...
        <div className="ai-message-toolbar">
          ...
          <div className="ai-toolbar-actions">
            ...
            />
            <button type="button" className="ai-button ai-button-light" onClick={openFilterDrawer}>
              筛选
            </button>
          </div>
        </div>
      ...
      {filterOpen ? (
        <div className="ai-overlay ai-overlay-right" onClick={() => closeFilterDrawer(true)}>
          <aside className="ai-message-drawer" onClick={(event) => event.stopPropagation()}>
            <div className="ai-message-drawer-header">
              <strong>筛选</strong>
              <button type="button" className="ai-close-button" onClick={() => closeFilterDrawer(true)}>
            ...
            <div className="ai-message-drawer-section">
              <label className="ai-mini-form">
                <span>日期</span>
                <input className="ai-input" type="date" value={draftFilter.day} onChange={(event) => setDraftFilter((current) => ({ ...current, day: event.target.value }))} />
              </label>
              <label className="ai-mini-form">
                <span>时刻</span>
                <input className="ai-input" type="number" min="0" max="23" value={draftFilter.hour} onChange={(event) => setDraftFilter((current) => ({ ...current, hour: event.target.value }))} />
              </label>
              <label className="ai-mini-form">
                <span>状态</span>
                <select className="ai-input ai-input-select" value={draftFilter.status} onChange={(event) => setDraftFilter((current) => ({ ...current, status: event.target.value as FilterState["status"] }))}>
              ...
              <label className="ai-mini-form">
                ...
                <select
                  ...
                  onChange={(event) => setDraftFilter((current) => ({ ...current, type: event.target.value as FilterState["type"] }))}
                >
            ...
            <div className="ai-modal-footer">
              ...
              <button type="button" className="ai-button ai-button-primary" onClick={applyFilterDrawer}>
                确定
              </button>
            </div>
          </aside>
